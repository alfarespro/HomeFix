...
class _EditAccountState extends State<EditAccount> {
    ...
    WidgetsBinding.instance.addPostFrameCallback((_) async {
      ...
      if (userData != null) {
        setState(() {
          ...
          _selectedArea = userData['area'] ?? '';
          _profileImageUrl = userData['profileImageUrl'];
        });
        final dataProvider = Provider.of<DataProvider>(context, listen: false);
        if (dataProvider.cities.contains(userData['city'])) {
          dataProvider.setCity(userData['city']);
        }
        if (dataProvider.areas.contains(userData['area'])) {
          dataProvider.setArea(userData['area']);
        }
      }
    });
  }
  ...
  Future<void> _addProfilePicture() async {
    ...
    if (image != null) {
      setState(() {
        ...
        _profileImageUrl = null; // Clear existing URL until new upload
      });
    } else {
  ...
  Future<void> _updateProfile() async {
    ...
    if (_profileImage != null) {
      newProfileImageUrl = await _uploadToCloudinary(_profileImage!);
    ...
    await _authService.updateUserProfile(
      ...
      onSuccess: () {
        ...
        if (mounted) {
          ...
          ScaffoldMessenger.of(context).showSnackBar(
            const SnackBar(content: Text('تم تعديل الملف بنجاح')),
          );
        }
      },
